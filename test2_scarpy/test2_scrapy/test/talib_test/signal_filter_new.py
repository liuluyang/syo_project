...
def signal():
    ...
    def data_kline_get():
        ...
        futures_url = 'https://www.okex.me/v2/futures/pc/market/klineData.do?' \
                      'symbol=f_usd_eos&type=3min&contractType=quarter&limit=1000&coinVol=1'#quarter
    ...
    while True:
        ...
        scale[end-1] = up
        true_change[end-1] = round(x, 3)
        start += 1

    data = list(zip(data_kline.index, round(change, 3), round(percent, 2),
                    scale, ma9, middle, close, true_change, high, low))
    ...
    for d in data:
        ...
        if abs(d[1]) >= 0.004 and abs(d[2]) >= 0.4 and d[3] < 40 and d[5] < d[4]:
            if is_buy >= 0:
                ...
                buy_price = price
        signal = ''
        if is_buy == 1 and (d[8] - buy_price >= 0.005):
            signal = '>>>>>>'
        if is_buy == -1 and (buy_price - d[9] >= 0.005):
            signal = '<<<<<<'
        print(d, signal)
    print(num)
