...
def get_datas(sql):
    # 一个传入sql导出数据的函数
    # 跟数据库建立连接
    conn = pms.connect(host='rm-rj91p2yhl9dm2xmbixo.mysql.rds.aliyuncs.com',
                       user='bi-analyzer',
                       passwd='pcNzcKPnn', database='kikuu', port=3306,
                       charset="utf8")
...
def main():
    ...
    my_sql = sql = "SELECT a.id ,\
           ...
           af.lastPayTime \
          FROM table a\
      LEFT JOIN table af ON a.id= af.accountId"
    # 生成数据
