...
class CurrencySpider(scrapy.Spider):
    ...
    def parse(self, response):
        currencies = response.css('#table_body tr')
    ...
    def content(self, response):
        ...
        quantity = cur_info[1].split(' ')[0]             #流通量
        circulation = cur_info[2].split(' ')[0]          #发行量
