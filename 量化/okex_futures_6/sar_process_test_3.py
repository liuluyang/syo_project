...
class SIGNAL(object):
    ...
    def signal(self):
        ...
        macd *= 2
        x_list = [0.003, 0.0025, -0.003, -0.0025]
        macd_last = macd[-1]
        print(last_time, macd_last, close_price, self.is_buy)

        signal_dict = {'开多':'1', '开空':'2', '平多':'3', '平空':'4'}
        signal_text = ''
        if macd_last > x_list[0] and self.is_buy != 1:
            signal_text = '开多'
        elif macd_last < x_list[1] and self.is_buy == 1:
            signal_text = '平多'
        elif macd_last < x_list[2] and self.is_buy != -1:
            signal_text = '开空'
        elif macd_last > x_list[3] and self.is_buy == -1:
            signal_text = '平空'
