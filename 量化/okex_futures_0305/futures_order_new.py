...
def check_position(**kwargs):
    ...
    if result.get('result') is True:
        ...
        #固定收益
        if long_qty > 0 and (long_pnl_ratio > 2 or long_pnl_ratio < -1.5):
            #平多
            print('平多')
            kwargs['leverage'] = long_leverage
            kwargs['size'] = long_qty
            futures_order(type='3', **kwargs)
        if short_qty > 0 and (short_pnl_ratio > 2 or short_pnl_ratio < -1.5):
            #平空
            print('平空')
        ...
        #     short_ratio = -2
        #     short_start_ratio = 1
        # print('多止损',long_ratio, long_start_ratio)
        # print('空止损', short_ratio, short_start_ratio)
