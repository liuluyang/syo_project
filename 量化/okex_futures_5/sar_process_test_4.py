...
class SIGNAL(object):
    ...
    def signal(self):
        ...
        for index, d in enumerate(list(zip(data_kline.index, macd, dif, middle, data_kline.high, data_kline.low, data_kline.close))):
            ...
            #         signal = '<<<<<<'
            # else:
            #     if (d[1] > x and d[2] < 0 and is_buy == -1) or (d[1] > 0.002 and d[2] > 0 and is_buy == -1):
            #         print('平空 开多')
            #         signal_text = '平空 开多'
            #         is_buy = 1
            #         p = price - d[3]
            ...
            #         price = d[3]
            #         signal = ('>>>>>>', p)
            #     elif (d[1] < -x and d[2] > 0 and is_buy == 1) or (d[1] < -0.002 and d[2] < 0 and is_buy == 1):
            #         print('平多 开空')
            #         signal_text = '平多 开空'
            #         is_buy = -1
            #         p = d[3] - price
